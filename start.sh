...
echo -e "${BLUE}[1/4] Cloning CR7 Games Proxy repository...${RESET}"
...
echo -e "${BLUE}[3/4] Starting CR7 Games Proxy...${RESET}"
npm start

if [ $? -eq 0 ]; then
    echo -e "${GREEN}[Success] CR7 Games Proxy started successfully.${RESET}"
    echo -e "${YELLOW}[Info] You can access Doge V4 via localhost.${RESET}"
else
    echo -e "${RED}[Error] Failed to start CR7 Games Proxy. Please check the application logs for details.${RESET}"
    exit 1
fi
